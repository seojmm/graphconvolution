...
import Total10, {
	...
	CAFES,
	PLACES,
} from "@/components/Total10/Total10";
...
type Message = {
	...
	places?: ReadonlyArray<PlaceInfo>;
};
...
const ChatContainer = ({ label = "채팅" }: { label?: string }) => {
	...
	const [messages, setMessages] = useState<Message[]>([
		{
			role: "assistant",
			content: "안녕하세요. 오늘은 어떤 약속을 계획하고 싶으신가요?",
		},
	]);
	...
	const [activePlaces, setActivePlaces] =
		useState<ReadonlyArray<PlaceInfo>>(PLACES);
	...
	const respondWithThinking = (
		...
		const timerId = window.setTimeout(() => {
			...
			pendingThinkingTimers.current = pendingThinkingTimers.current.filter(
				(id) => id !== timerId
			);
		}, delay);
	...
	const handleReservationDecision = (
		messageIndex: number,
	...
	const handleRetryDecision = (
		...
		if (restart) {
			...
			respondWithThinking({
				content:
					"처음으로 돌아갈게요. 더 정확한 정보 제공을 위해서, 몇 시쯤에 가실 예정인가요?",
	...
	const handleScenarioFlow = (query: string): boolean => {
		...
		if (normalized.includes("달달")) {
			setSuggestions([]);
			setActivePlaces(CAFES);
			setScenarioStep("resultsShown");
			respondWithThinking({
				content:
					"맛있는 디저트가 있는 카페를 추천드릴까요? 딸기나 말차 디저트 어때요?",
				variant: "totalResults",
				places: CAFES,
			});
			return true;
		...
		if (scenarioStep === "resultsShown" && normalized.includes("별론데")) {
			setScenarioStep("awaitingReversePreference");
			setSuggestions([]);
			respondWithThinking({
				content: "다시 추천해드릴까요? 음식 종류는 상관 없으신가요?",
			});
			return true;
		}

		if (
			scenarioStep === "awaitingReversePreference" &&
			normalized.includes("상관없어")
		) {
			setScenarioStep("resultsShown");
			setSuggestions([]);
			respondWithThinking({
				content: "총 10곳을 다시 추천드릴게요.",
				variant: "totalResults",
				reverseOrder: true,
				places: activePlaces,
			});
		...
		if (
			(scenarioStep === "idle" || scenarioStep === "resultsShown") &&
			...
			respondWithThinking({
				content:
					"저도 영화 좋아해요! 혹시 조용한 분위기의 장소를 찾고 계신가요?",
			});
			return true;
		}
		...
		if (
			...
			respondWithThinking({
				content:
					"피곤하시겠어요. 이동 시간과 상황에 맞는 약속 장소를 바로 찾아볼게요.",
			});
			return true;
		}
		...
		try {
			const res = await fetch(`${API_BASE}/chat`, {
				...
				body: JSON.stringify({
					...
					history: historyForRequest.map((msg) => ({
						role: msg.role,
						content: msg.content,
						meta: msg.meta,
					})),
			...
			const reply =
				typeof data?.reply === "string"
					? data.reply
					: JSON.stringify(data?.reply ?? data, null, 2);
			...
			if (Array.isArray(metaObj?.extraQueries) && metaObj.extraQueries.length) {
				metaParts.push(
					`tools: web_search -> ${metaObj.extraQueries.join(" | ")}`
				);
			}
			...
			const explicitSuggestionsTop = normalizeSuggestionTemplates(
				data?.suggestions
			);
			const explicitSuggestionsMeta = normalizeSuggestionTemplates(
				metaObj?.suggestions
			);
			const agentObj = asRecord(data?.agent);
			const explicitSuggestionsAgent = normalizeSuggestionTemplates(
				agentObj?.suggestions
			);
						...
						{msg.isLoading ? (
							...
							<div className="flex flex-col gap-3">
								...
								<Total10
									...
									onExpandToggle={handleListExpandToggle}
									reverseOrder={msg.reverseOrder}
									items={msg.places}
								/>
							</div>
								...
								<div className="flex gap-2">
									{(msg.choices ?? []).map((choice) => (
										<button
											...
											type="button"
											onClick={() =>
												handleRetryDecision(idx, msg.place, choice === "네")
											}
											disabled={!msg.choices?.length}
											className={`rounded-full px-3 py-1 text-xs font-semibold transition ${
												choice === "네"
